...
class ZoteroDBAnalyzer {
  ...
  findVariantsEfficiently(surnameFrequencies, progressCallback = null, shouldCancel = null) {
    ...
    // Only look for diacritic-only variants, not typos
    // Bennett vs Dennett should NOT be matched, but Müller vs Muller should
    let comparisons = 0;
    ...
    for (let i = 0; i < surnames.length; i++) {
      ...
      // Only compare with names that come after (to avoid duplicates)
      for (let j = i + 1; j < surnames.length; j++) {
        const name2 = surnames[j];

        // Quick length check - must be same length for diacritic-only variant
        if (name1.length !== name2.length) continue;

        comparisons++;

        // Only check if they are diacritic-only variants
        // This is much faster than calculating Levenshtein distance
        if (isDiacriticOnlyVariant(name1, name2)) {
          potentialVariants.push({
            ...
            similarity: 1.0, // Diacritic-only variants are essentially the same name
      ...
      if (progressCallback && i % 10 === 0) {
        progressCallback({
          ...
          percent: Math.round((i / surnames.length) * 100)
        });
      }
    }
